<?php
	...
	function process_save_pageview(){
		global $mysqli;
		...
		$user_token = get_current_page_token();
		$src = $_SERVER["HTTP_REFERER"];
		$page_id = $_REQUEST['user_page_id'];
		$res = $mysqli->query("SELECT user_id FROM pages WHERE id=$page_id");
		if($res != false){
			$arr = $res->fetch_array( MYSQLI_ASSOC );
			$location = json_decode( get_curl_data( 'http://ip-api.com/json/' . $_REQUEST['ip'] ) );
			$mysqli->query("INSERT INTO pageviews (date, token, user_id, page_id, src, ip, platform, browser, device, browser_info, city, country, zip_code, latitude, longitude ) VALUES ('" . date("Y-m-d H:i:s") . "', '" . $user_token . "', '" . $arr['user_id'] . "', '" . $page_id . "', '" . $src . "', '" . $_REQUEST['ip'] . "', '" . $mysqli->real_escape_string( $platform ) . "', '" . $mysqli->real_escape_string( $browser ) . "', '" . $device . "', '" . $mysqli->real_escape_string( $_REQUEST['agent'] ) . "', '" . $location->city . "', '" . $location->country . "', '" . $location->zip . "', '" . $location->lat . "', '" . $location->lon . "')");
			echo $mysqli->error;
		}
